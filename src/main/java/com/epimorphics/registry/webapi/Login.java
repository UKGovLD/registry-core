...
public class Login {
    ...
    public static final String SA_IDENTITY = "user_identity";
    public static final String SA_NAME = "user_name";
...
//                fetch.addAttribute(AP_EMAIL, "http://schema.openid.net/contact/email", false);
    ...
    public Response openIDResponse(@Context HttpServletRequest request) {
        Identifier identity = verifyResponse(request);
        if (identity != null) {
            return RequestProcessor.render("welcome.vm", uriInfo, servletContext, request, "identity", identity);
        } else {
            return RequestProcessor.render("error.vm", uriInfo, servletContext, request, "message", "Failed to verify identity");
        }
    }
        ...
        try {
            ...
            if (verified != null) {
                ...
                if (authSuccess.hasExtension(AxMessage.OPENID_NS_AX)) {
                    ...
                    session.setAttribute(SA_NAME, name);
                    session.setAttribute(SA_IDENTITY, verified.getIdentifier());
                    return verified;
                }
            }
        } catch (Exception e) {
